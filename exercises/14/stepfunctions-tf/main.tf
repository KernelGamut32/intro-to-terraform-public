...
# Create the function
data "archive_file" "true_route" {
    type        = "zip"
    source_file = "src/true-route.py"
    output_path = "src/true-route.zip"
}

resource "aws_lambda_function" "true_route_lambda" {
    function_name    = "TrueRouteFunction-${random_string.random.id}"
    role             = aws_iam_role.function_role.arn
    handler          = "true-route.lambda_handler"
    runtime          = "python3.9"
    filename         = "src/true-route.zip"
    source_code_hash = data.archive_file.lambda.output_base64sha256
}

# Explicitly create the function's log group to set retention and allow auto-cleanup
resource "aws_cloudwatch_log_group" "true_route_function_log" {
    retention_in_days = 1
    name              = "/aws/lambda/${aws_lambda_function.true_route_lambda.function_name}"
}

# Create the function
data "archive_file" "false_route" {
    type        = "zip"
    source_file = "src/false-route.py"
    output_path = "src/false-route.zip"
}

resource "aws_lambda_function" "false_route_lambda" {
    function_name    = "FalseRouteFunction-${random_string.random.id}"
    role             = aws_iam_role.function_role.arn
    handler          = "false-route.lambda_handler"
    runtime          = "python3.9"
    filename         = "src/false-route.zip"
    source_code_hash = data.archive_file.lambda.output_base64sha256
}

# Explicitly create the function's log group to set retention and allow auto-cleanup
resource "aws_cloudwatch_log_group" "false_route_function_log" {
    retention_in_days = 1
    name              = "/aws/lambda/${aws_lambda_function.false_route_lambda.function_name}"
}
...
data "template_file" "SFDefinitionFile" {
    ...
    vars = {
        DecisionFunction  = aws_lambda_function.test_lambda.arn
        TrueFunction = aws_lambda_function.true_route_lambda.arn
        FalseFunction = aws_lambda_function.false_route_lambda.arn
    }
}
